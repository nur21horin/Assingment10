...
const ManageMyFoods = () => {
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      ...
      setMyFoods(myFoods.map((food) => (food._id === editingFood ? form : food)));
      setEditingFood(null);
  ...
  return (
    <section className="max-w-4xl mx-auto py-12 px-4 transition-colors duration-300 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100">
      ...
      {loading ? (
        ...
        <p className="text-center text-gray-500 dark:text-gray-400">No foods added yet.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {myFoods.map((food) => (
            <div
              key={food._id}
              className="bg-white dark:bg-gray-800 shadow-lg dark:shadow-gray-700 rounded-3xl overflow-hidden transition hover:shadow-2xl"
            >
              <img
                src={food.food_image}
                alt={food.food_name}
      ...
      {editingFood && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-start pt-20 px-4 z-50 overflow-auto">
          <form
            ...
            className="bg-white dark:bg-gray-800 shadow-2xl rounded-3xl p-10 max-w-md w-full space-y-6 transition-colors duration-300"
          >
            <h3 className="text-3xl font-extrabold text-green-600 text-center">
              Update Food
            </h3>

            {["food_name", "food_quantity", "pickup_location", "expire_date", "additional_notes"].map((field) => (
              <div key={field}>
                ...
                {field === "additional_notes" ? (
                  <textarea
                    ...
                    rows="3"
                    className="border rounded-xl p-3 w-full bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-green-400 focus:outline-none transition"
                  />
                ) : field === "expire_date" ? (
                  <input
                    type="date"
            ...
            <div>
              ...
              <input
                ...
                className="border rounded-xl p-2 w-full cursor-pointer bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 focus:ring-2 focus:ring-green-400 focus:outline-none transition"
              />
              {imagePreview && (
                <img
                  src={imagePreview}
